...
func (jp *JsonPrinter) Print(r io.Reader, w io.Writer) {
	...
	for scanner.Scan() {
		line := scanner.Text()
		printLineAsJsonFormat(line, w)
	}
}

func printLineAsJsonFormat(line string, w io.Writer) {
	...
	if len(splitted) == 1 {
		...
		fmt.Fprintf(w, "%s%s\n", indent, toColorizedJsonValue(splitted[0]))
		return
	}
	...
	fmt.Fprintf(w, "%s%s: %s\n", indent, toColorizedJsonKey(key, indentCnt, 4), toColorizedJsonValue(val))
}

// toColorizedJsonKey returns colored json key
func toColorizedJsonKey(key string, indentCnt, basicWidth int) string {
...
func toColorizedJsonValue(value string) string {
	if value == "{" {
		return "{"
	}
	...
	return fmt.Sprintf(format, color.Apply(doubleQuoteTrimmedValue, getColorByValueType(value)))
}
