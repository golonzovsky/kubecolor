...
func (vsp *VersionShortPrinter) Print(r io.Reader, w io.Writer) {
	...
	for scanner.Scan() {
		...
		fmt.Fprintf(w, "%s: %s\n",
			color.Apply(key, getColorByKeyIndent(0, 2)),
			color.Apply(val, getColorByValueType(val)),
		)
	}
}
...
func (vp *VersionPrinter) Print(r io.Reader, w io.Writer) {
	...
	for scanner.Scan() {
		...
		fmt.Fprintf(w, "%s: %s{", key, color.Apply(packageName, getColorByKeyIndent(2, 2)))
		for i, value := range values {
			kv := strings.SplitN(value, ":", 2)
			coloredKey := color.Apply(kv[0], getColorByKeyIndent(0, 2))

			isValDoubleQuotationSurrounded := strings.HasPrefix(kv[1], `"`) && strings.HasSuffix(kv[1], `"`)
			val := strings.TrimRight(strings.TrimLeft(kv[1], `"`), `"`)

			coloredVal := color.Apply(val, getColorByValueType(kv[1]))
